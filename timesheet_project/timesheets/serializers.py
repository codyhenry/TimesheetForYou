...
class TimeEntrySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ["timesheet", "total_hours", "signature_status", "created_at", "updated_at"]
    ...
    def validate(self, attrs):
        timesheet = self.context.get("timesheet") or getattr(self.instance, "timesheet", None)
        work_date = attrs.get("work_date", getattr(self.instance, "work_date", None))
        start_time = attrs.get("start_time", getattr(self.instance, "start_time", None))
        end_time = attrs.get("end_time", getattr(self.instance, "end_time", None))
        family_name = attrs.get("family_name", getattr(self.instance, "family_name", ""))

        if timesheet and work_date and not (timesheet.week_start_date <= work_date <= timesheet.week_end_date):
            raise serializers.ValidationError({"work_date": "Entry date must fall within the timesheet week."})
        if family_name is not None and not str(family_name).strip():
            raise serializers.ValidationError({"family_name": "Family name is required."})
...
class TimesheetSubmissionSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ["id", "status", "submitted_at", "total_hours", "pdf_file", "snapshot"]
        read_only_fields = fields
...
class WeeklyTimesheetListSerializer(serializers.ModelSerializer):
    ...
    def _entries(self, obj):
        prefetched = getattr(obj, "_prefetched_objects_cache", {}).get("entries")
...
class WeeklyTimesheetDetailSerializer(WeeklyTimesheetListSerializer):
    ...
    class Meta(WeeklyTimesheetListSerializer.Meta):
        fields = WeeklyTimesheetListSerializer.Meta.fields + ["entries", "submission"]
...
class AdminTimesheetListSerializer(WeeklyTimesheetListSerializer):
    ...
    class Meta(WeeklyTimesheetListSerializer.Meta):
        fields = ["id", "nanny", "week_start_date", "week_end_date", "status", "submitted_at", "total_hours", "signed_entry_count", "unsigned_entry_count"]
...
class AdminTimesheetDetailSerializer(WeeklyTimesheetDetailSerializer):
    ...
    class Meta(WeeklyTimesheetDetailSerializer.Meta):
        fields = ["id", "nanny", "week_start_date", "week_end_date", "status", "submitted_at", "admin_notes", "total_hours", "signed_entry_count", "unsigned_entry_count", "total_hours_by_family", "entries", "submission"]
